...
describe('leadershipRouter', () => {
	...
	describe('PATCH /:id', () => {
		it('should return 200 with an existing leadership role id', (done) => {
			sandbox.stub(leadershipService, 'updateLeadershipRole').resolves(true);

			request(app)
				.patch('/api/leadership/1')
				.set('Cookie', `accessToken=${token}`)
				.expect('Content-Type', /json/)
				.expect(200)
				.end((err, res) => {
		...
		it('should return 404 when the leadership role id doesnt exist in the db', (done) => {
			sandbox.stub(leadershipService, 'updateLeadershipRole').resolves(false);
		...
		it('should return 400 for a non-number leadership role id', (done) => {
			const expectedResBody = {
				ok: false,
				status: 400,
				error: [
