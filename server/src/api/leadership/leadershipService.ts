...
interface LeadershipRoleDefaultStrings {
	name: string;
	attribute: string;
	benefit: string;
}

const roleDefaultStrings: LeadershipRoleDefaultStrings[] = [
...
async function createLeadershipRoles(kingdom: KingdomEntity): Promise<void> {
	...
	const roles = Array.from({ length: 13 }, (_, i) => {
		...
		role.name = roleDefaultStrings[i].name;
		role.heldBy = '';
		role.attribute = roleDefaultStrings[i].attribute;
		role.abilityBonus = 0;
		role.leadership = false;
		role.benefit = roleDefaultStrings[i].benefit;
		role.vacant = true;
		role.skillBonus = 0;
...
async function updateLeadershipRole(
	id: number,
	updates: Partial<LeadershipRole>
): Promise<boolean> {
	const leadershipRepository = getRepository(LeadershipEntity);

	const role = await leadershipRepository.findOne(id);

	if (!role) {
		return false;
	}

	role.name = updates.name ?? role.name;
	role.heldBy = updates.heldBy ?? role.heldBy;
	role.attribute = updates.attribute ?? role.attribute;
	role.abilityBonus = updates.abilityBonus ?? role.abilityBonus;
	role.leadership = updates.leadership ?? role.leadership;
	role.benefit = updates.benefit ?? role.benefit;
	role.vacant = updates.vacant ?? role.vacant;
	role.skillBonus = updates.skillBonus ?? role.skillBonus;

	await leadershipRepository.save(role);
